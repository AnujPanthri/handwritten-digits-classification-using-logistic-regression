...
model=one_for_all_logistic_regression(x_train,y_train)
model.train_all(iter=2000,alpha=0.1,order=1,reg=0,show=False)
model.acc()
# model.decision_boundary()

processed_X=model.all_models[0].ready_data(x_test)
out=model.get_results(processed_X)
